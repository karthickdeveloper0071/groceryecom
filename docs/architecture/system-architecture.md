...
```
 ...
 │ primary       │              └───────────────┘
 │  + read       │              ┌───────────────┐
 │    replica    │              │ RabbitMQ      │  not used by code yet
 └───────────────┘              └───────────────┘
```
...
| Messaging | RabbitMQ | wired in config; **no code publishes to it yet** |
...
- Health groups: readiness includes only `readinessState` and `db`, so a Redis or
  RabbitMQ outage degrades features but does not remove an instance from the load
...
| RabbitMQ is configured but unused | module events are in-process plus the outbox table |
